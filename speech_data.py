...
def maybe_download(file, work_directory):
  ...
  if not os.path.exists(filepath):
    ...
    filepath, _ = six.moves.urllib.request.urlretrieve(url_filename, filepath,progresshook)
...
class DataSet(object):
  ...
  def next_batch(self, batch_size, fake_data=False):
    ...
    if fake_data:
      fake_image = [1] * Source.width * Source.height
      if self.one_hot:
        fake_label = [1] + [0] * 9
      else:
        fake_label = 0
...
def dense_to_one_hot(batch, batch_size, num_labels):
  sparse_labels = Source.tf.reshape(batch, [batch_size, 1])
  indices = Source.tf.reshape(Source.tf.range(0, batch_size, 1), [batch_size, 1])
  concatenated = Source.tf.concat(1, [indices, sparse_labels])
  concat = Source.tf.concat(0, [[batch_size], [num_labels]])
  output_shape = Source.tf.reshape(concat, [2])
  sparse_to_dense = Source.tf.sparse_to_dense(concatenated, output_shape, 1.0, 0.0)
  return Source.tf.reshape(sparse_to_dense, [batch_size, num_labels])
...
def read_data_sets(train_dir,source_data=Source.NUMBER_IMAGES, fake_data=False, one_hot=True):
  ...
  train_images = extract_images(Source.TRAIN_INDEX,train=True)
  train_labels = extract_labels(Source.TRAIN_INDEX,train=True, one_hot=one_hot)
  test_images = extract_images(Source.TEST_INDEX,train=False)
  test_labels = extract_labels(Source.TEST_INDEX,train=False, one_hot=one_hot)
